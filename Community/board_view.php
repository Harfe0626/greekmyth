...
<body>
    <div id="wrap">
        ...
        <section id="sub_contents">
            <h3>공지사항<span>HOME>커뮤니티>공지사항</span></h3>

            <?php
            $con = mysqli_connect("localhost", "user1", "12345", "sample");
            $num = $_GET['num'];
            $query = "select subject, content, regist_day, hit, id from board_notice where num = $num";
            ...
            <div class="read_btn">
                <button class="read_btn1" onclick="location.href='Notice.php'">목록</button>&nbsp;&nbsp;
                <button class="read_btn1" onclick="location.href='board_modify.php?num=<?= $num ?>'">수정</button>&nbsp;&nbsp;
                <button class="read_btn1" a onclick="ask();">삭제</button>
                <script> 
                    function ask() {
                        if (confirm("게시글을 삭제하시겠습니까?")) {
                            window.location = "board_delete.php?num=<?= $num ?>"
                        }
                    }
                </script>
            </div>
